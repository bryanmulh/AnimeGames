...
def selected_deck_from_query(path):
...
def deck_editor_html(path):
    selected = selected_deck_from_query(path)
    deck_tabs = []
...
class CombinedHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path.startswith("/assets/"):
            self.serve_combined_asset(parsed.path)
            return
        if parsed.path == "/decks":
            self.send_deck_editor()
            return
    ...
    def send_deck_editor(self):
        body = deck_editor_html(self.path)
        self.send_response(200)
        self.send_header("Content-Type", "text/html; charset=utf-8")
